...
public class DecayTaskManager
{
	...
	{
		removeFromTasks(actor);
	}
	
	/**
	 * @param actor
	 */
	private void removeFromTasks(L2Character actor)
	{
		try
		{
		...
		public void run()
		{
			...
			try
			{
				for (L2Character actor : keySet)
				{
					...
					Long currentForActor = _decayTasks.get(actor);
					if (currentForActor == null)
					{
						continue;
					}
					
					if ((current - currentForActor) > delay)
					{
						actor.onDecay();
						cancelDecayTask(actor);
					}
				}
			}
			catch (Throwable e)
