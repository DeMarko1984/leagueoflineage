...
public class AdminEventEngine implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_lge_list_events"))
		{
			LgeEvent[] currentEvents = LgeManager.getInstance().getCurrentEvents();
			for (LgeEvent lgeEvent : currentEvents)
			{
				String eventData = "=================================\n" + "EventType: " + lgeEvent.getEventType() + "\n" + "InstanceId: " + lgeEvent.getEventInstanceId() + "\n" + "TeamsMidLevel: "
					+ lgeEvent.getTeamsMidLevel() + "\n" + "ParticipatedPlayersCount: " + lgeEvent.getParticipatedPlayersCount() + "\n" + "State: " + lgeEvent.getState() + "\n" + "TimeMod: "
					+ lgeEvent.timeMod();
				activeChar.sendMessage(eventData);
			}
		}
	...
	String showStoredEvents()
	{
		...
		for (String file2 : files)
		{
			...
			result += "<font color=\"LEVEL\">" + file.getName() + " </font><br><button value=\"select\" action=\"bypass -h admin_event_set " + file.getName()
				+ "\" width=90 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"><button value=\"ver\" action=\"bypass -h admin_event_see " + file.getName()
				+ "\" width=90 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\"><button value=\"delete\" action=\"bypass -h admin_event_del " + file.getName()
	...
	public void showEventParameters(L2PcInstance activeChar, int teamnumbers)
	{
		...
		replyMSG
			.append(
				"<br><br>Announcer NPC id<edit var=\"event_npcid\" width=100 height=20><br><br><button value=\"Announce Event!!\" action=\"bypass -h admin_event_announce $event_npcid " + teamnumbers
					+ " ");
		for (int i = 0; i < teamnumbers; i++)
		{
	...
	void muestraNpcConInfoAPlayers(L2PcInstance activeChar, int id)
	{
		...
		for (L2PcInstance player : L2World.getInstance().getAllPlayers())
		{
			...
			for (L2PcInstance playertemp : player.getKnownList().getKnownPlayersArray())
			{
