...
public class AdminFightCalculator implements IAdminCommandHandler
{
	...
	private void handleStart(String params, L2PcInstance activeChar)
	{
		...
		if ((npc1 != null) && (npc2 != null))
		{
			...
			replyMSG
				.append("<button value=\"OK\" action=\"bypass -h admin_fight_calculator_show " + npc1.npcId + " " + npc2.npcId + "\"  width=100 height=15 back=\"sek.cbui94\" fore=\"sek.cbui92\">");
	...
	private void handleShow(String params, L2PcInstance activeChar)
	{
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			boolean _crit1 = f.calcCrit(npc1, npc1.getCriticalHit(npc2, null), false);
			if (_crit1)
			{
				crit1++;
			}
		...
		for (int i = 0; i < 10000; i++)
		{
			...
			boolean _crit2 = f.calcCrit(npc2, npc2.getCriticalHit(npc1, null), false);
			if (_crit2)
			{
				crit2++;
			}
		...
		{
			replyMSG
				.append("<tr><td width=140>Parameter</td><td width=70>" + ((L2NpcTemplate) npc1.getTemplate()).name + "</td><td width=70>" + ((L2NpcTemplate) npc2.getTemplate()).name + "</td></tr>");
		}
		...
		else
		{
			replyMSG
				.append(
					"<button value=\"Retry\" action=\"bypass -h admin_fight_calculator_show " + ((L2NpcTemplate) npc1.getTemplate()).npcId + " " + ((L2NpcTemplate) npc2.getTemplate()).npcId
