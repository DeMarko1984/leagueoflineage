...
public class Blow implements ISkillHandler
{
	...
	public void useSkill(L2Character activeChar, L2Skill skill, L2Object[] targets)
	{
		...
		for (L2Object target2 : targets)
		{
			...
			if ((((skill.getCondition() & L2Skill.COND_BEHIND) != 0) && (_successChance == BEHIND))
				|| (((skill.getCondition() & L2Skill.COND_CRIT) != 0) && Formulas.getInstance().calcBlow(activeChar, target, _successChance)))
			{
				if (skill.hasEffects())
				{
				...
				boolean crit = false;
				if (Formulas.getInstance().calcCrit(activeChar, skill.getBaseCritRate() * 10 * Formulas.getInstance().getSTRBonus(activeChar), true))
				{
					crit = true;
				}
			...
			if (!target.isRaid() && !(target instanceof L2DoorInstance) && !((target instanceof L2NpcInstance) && (((L2NpcInstance) target).getNpcId() == 35062)))
			{
				...
				if ((skill.getLethalChance2() > 0) && Formulas.getInstance().calcLethal(activeChar, target, skill.getLethalChance2()))
				{
				...
				else if ((skill.getLethalChance1() > 0) && Formulas.getInstance().calcLethal(activeChar, target, skill.getLethalChance1()))
				{
