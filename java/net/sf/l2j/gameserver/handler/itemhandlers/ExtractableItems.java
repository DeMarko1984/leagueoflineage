...
public class ExtractableItems implements IItemHandler
{
	...
	public void useItem(L2PlayableInstance playable, L2ItemInstance item)
	{
		...
		if (Rnd.nextBoolean())
		{
			...
			L2ExtractableProductItem[] expi = exitem.getProductItemsArray();
			L2ExtractableProductItem eitem = expi[Rnd.get(0, expi.length - 1)];
			createItemID = eitem.getId();
			createAmount = eitem.getAmmount();
		}
		else
		{
			createItemID = 57;
			int level = activeChar.getLevel();
			createAmount = Rnd.get(level * 100, level * 200);
		}
		...
		if (createItemID == 57)
		{
			activeChar.addAdena("Extract", createAmount, item, true);
		}
		else if (createItemID > 0)
		{
