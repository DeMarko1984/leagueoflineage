...
import net.sf.l2j.gameserver.model.entity.L2CubicContainer;
...
public final class L2PcInstance extends L2PlayableInstance
{
	...
	private static final String ADD_SKILL_SAVE =
		"INSERT INTO character_skills_save (char_obj_id,skill_id,skill_level,effect_count,effect_cur_time,reuse_delay,restore_type,class_index,buff_index) VALUES (?,?,?,?,?,?,?,?,?)";
	private static final String RESTORE_SKILL_SAVE =
		"SELECT skill_id,skill_level,effect_count,effect_cur_time, reuse_delay FROM character_skills_save WHERE char_obj_id=? AND class_index=? AND restore_type=? ORDER BY buff_index ASC";
	private static final String DELETE_SKILL_SAVE = "DELETE FROM character_skills_save WHERE char_obj_id=? AND class_index=?";
	private static final String UPDATE_CHARACTER =
		"UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,str=?,con=?,dex=?,_int=?,men=?,wit=?,face=?,hairStyle=?,hairColor=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,pvpkills=?,pkkills=?,rec_have=?,rec_left=?,clanid=?,maxload=?,race=?,classid=?,deletetime=?,title=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,in_jail=?,jail_timer=?,newbie=?,nobless=?,power_grade=?,subpledge=?,last_recom_date=?,lvl_joined_academy=?,apprentice=?,sponsor=?,varka_ketra_ally=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,death_penalty_level=?,_add_str=?,_add_dex=?,_add_con=?,_add_wit=?,_add_men=?,_add_int=? WHERE obj_id=?";
	private static final String RESTORE_CHARACTER =
		"SELECT account_name, obj_Id, char_name, level, maxHp, curHp, maxCp, curCp, maxMp, curMp, acc, crit, evasion, mAtk, mDef, mSpd, pAtk, pDef, pSpd, runSpd, walkSpd, str, con, dex, _int, men, wit, face, hairStyle, hairColor, sex, heading, x, y, z, movement_multiplier, attack_speed_multiplier, colRad, colHeight, exp, expBeforeDeath, sp, karma, pvpkills, pkkills, clanid, maxload, race, classid, deletetime, cancraft, title, rec_have, rec_left, accesslevel, online, char_slot, lastAccess, clan_privs, wantspeace, base_class, onlinetime, isin7sdungeon, in_jail, jail_timer, newbie, nobless, power_grade, subpledge, last_recom_date, lvl_joined_academy, apprentice, sponsor, varka_ketra_ally,clan_join_expiry_time,clan_create_expiry_time,death_penalty_level,_add_str,_add_dex,_add_con,_add_wit,_add_men,_add_int FROM characters WHERE obj_id=?";
	...
	private volatile L2GameClient _client;
	private volatile String _accountName;
	private volatile long _deleteTimer;
	private volatile boolean _isOnline = false;
	private volatile long _onlineTime;
	private volatile long _onlineBeginTime;
	private volatile long _lastAccess;
	private volatile long _uptime;
	protected volatile int _baseClass;
	protected volatile int _activeClass;
	protected volatile int _classIndex = 0;
	private Map<Integer, SubClass> _subClasses;
	private PcAppearance _appearance;
	private int _charId = 0x00030b7a;
	
	/** The Experience of the L2PcInstance before the last Death Penalty. */
	private volatile long _expBeforeDeath;
	
	/** The Karma of the L2PcInstance (if higher than 0, the name of the L2PcInstance appears in red). */
	private volatile int _karma;
	
	/** The number of player killed during a PvP (the player killed was PvP Flagged). */
	private volatile int _pvpKills;
	
	/** The PK counter of the L2PcInstance (= Number of non PvP Flagged player killed). */
	private volatile int _pkKills;
	
	/** The PvP Flag state of the L2PcInstance (0=White, 1=Purple). */
	private volatile byte _pvpFlag;
	
	/** The Siege state of the L2PcInstance. */
	private volatile byte _siegeState = 0;
	
	/** The _cur weight penalty. */
	private volatile int _curWeightPenalty = 0;
	
	/** The _last compass zone. */
	private volatile int _lastCompassZone; // the last compass zone update send to the client
	
	/** The _zone validate counter. */
	private volatile byte _zoneValidateCounter = 4;
	
	/** The _is in7s dungeon. */
	private volatile boolean _isIn7sDungeon = false;
	
	/** The _in jail. */
	private volatile boolean _inJail = false;
	
	/** The _jail timer. */
	private volatile long _jailTimer = 0;
	
	/** The _jail task. */
	private ScheduledFuture<?> _jailTask;
	
	/** Olympiad. */
	private volatile boolean _inOlympiadMode = false;
	
	/** The _ olympiad start. */
	private volatile boolean _OlympiadStart = false;
	
	/** The _olympiad game id. */
	private volatile int _olympiadGameId = -1;
	
	/** The _olympiad side. */
	private volatile int _olympiadSide = -1;
	
	/** Duel. */
	private volatile boolean _isInDuel = false;
	
	/** The _duel state. */
	private volatile int _duelState = Duel.DUELSTATE_NODUEL;
	
	/** The _duel id. */
	private volatile int _duelId = 0;
	
	/** The _no duel reason. */
	private SystemMessageId _noDuelReason = SystemMessageId.THERE_IS_NO_OPPONENT_TO_RECEIVE_YOUR_CHALLENGE_FOR_A_DUEL;
	
	/** Boat. */
	private volatile boolean _inBoat;
	...
	/** The _mount type. */
	private volatile int _mountType;
	
	/** Store object used to summon the strider you are mounting *. */
	private volatile int _mountObjectID = 0;
	
	/** The _telemode. */
	public volatile int _telemode = 0;
	
	/** The _exploring. */
	public volatile boolean _exploring = false;
	
	/** The _is silent moving. */
	private volatile boolean _isSilentMoving = false;
	
	/** The _in crystallize. */
	private volatile boolean _inCrystallize;
	
	/** The _in craft mode. */
	private volatile boolean _inCraftMode;
	
	/** The table containing all L2RecipeList of the L2PcInstance. */
	private final HashMap<Integer, L2RecipeList> _dwarvenRecipeBook = new HashMap<>();
	
	/** The _common recipe book. */
	private final HashMap<Integer, L2RecipeList> _commonRecipeBook = new HashMap<>();
	
	/** True if the L2PcInstance is sitting. */
	private volatile boolean _waitTypeSitting;
	
	/** True if the L2PcInstance is using the relax skill. */
	private volatile boolean _relax;
	
	/** Location before entering Observer Mode. */
	private volatile int _obsX;
	
	/** The _obs y. */
	private volatile int _obsY;
	
	/** The _obs z. */
	private volatile int _obsZ;
	
	/** The _observer mode. */
	private volatile boolean _observerMode = false;
	...
	private volatile int _recomHave; // how much I was recommended by others
	
	/** The number of recommendation that the L2PcInstance can give. */
	private volatile int _recomLeft; // how many recommendations I can give to others
	
	/** Date when recommendation points were updated last time. */
	private volatile long _lastRecomUpdate;
	...
	private volatile int _privatestore;
	...
	private volatile boolean _newbie;
	
	/** The _noble. */
	private volatile boolean _noble = false;
	
	/** The _hero. */
	private volatile boolean _hero = false;
	...
	/** The _henna str. */
	private volatile int _hennaSTR;
	
	/** The _henna int. */
	private volatile int _hennaINT;
	
	/** The _henna dex. */
	private volatile int _hennaDEX;
	
	/** The _henna men. */
	private volatile int _hennaMEN;
	
	/** The _henna wit. */
	private volatile int _hennaWIT;
	
	/** The _henna con. */
	private volatile int _hennaCON;
	
	/** The custrom adds for base */
	private volatile int _addSTR;
	private volatile int _addINT;
	private volatile int _addDEX;
	private volatile int _addMEN;
	private volatile int _addWIT;
	private volatile int _addCON;
	
	/** The custrom adds for weapon type skill */
	private final int[] _addWeaponUseSkill = new int[L2WeaponType.values().length];
	private volatile int _addMagicUseSkill;
	
	/** The L2Summon of the L2PcInstance. */
	private volatile L2Summon _summon = null;
	// apparently, a L2PcInstance CAN have both a summon AND a tamed beast at the same time!!
	/** The _tamed beast. */
	private volatile L2TamedBeastInstance _tamedBeast = null;
	...
	private volatile boolean _partyMatchingAutomaticRegistration;
	
	/** The _party matching show level. */
	private volatile boolean _partyMatchingShowLevel;
	
	/** The _party matching show class. */
	private volatile boolean _partyMatchingShowClass;
	
	/** The _party matching memo. */
	private volatile String _partyMatchingMemo;
	...
	private volatile L2Clan _clan;
	
	/** Apprentice and Sponsor IDs. */
	private volatile int _apprentice = 0;
	
	/** The _sponsor. */
	private volatile int _sponsor = 0;
	
	/** The _clan join expire time. */
	private volatile long _clanJoinExpiryTime;
	
	/** The _clan create expire time. */
	private volatile long _clanCreateExpiryTime;
	
	/** The _power grade. */
	private volatile int _powerGrade = 0;
	
	/** The _clan privileges. */
	private volatile int _clanPrivileges = 0;
	
	/** L2PcInstance's pledge class (knight, Baron, etc.) */
	private volatile int _pledgeClass = 0;
	
	/** The _pledge type. */
	private volatile int _pledgeType = 0;
	
	/** Level at which the player joined the clan as an academy member. */
	private volatile int _lvlJoinedAcademy = 0;
	
	/** The _wants peace. */
	private volatile int _wantsPeace = 0;
	
	// Death Penalty Buff Level
	/** The _death penalty buff level. */
	private volatile int _deathPenaltyBuffLevel = 0;
	
	// GM related variables
	/** The _isGm. */
	private volatile boolean _isGm;
	
	/** The _access level. */
	private volatile int _accessLevel;
	
	/** The _chat banned. */
	private volatile boolean _chatBanned = false; // Chat Banned
	
	/** The _chat unban task. */
	private ScheduledFuture<?> _chatUnbanTask = null;
	
	/** The _message refusal. */
	private volatile boolean _messageRefusal = false; // message refusal mode
	
	/** The _diet mode. */
	private volatile boolean _dietMode = false; // ignore weight penalty
	
	/** The _trade refusal. */
	private volatile boolean _tradeRefusal = false; // Trade refusal
	
	/** The _exchange refusal. */
	private volatile boolean _exchangeRefusal = false; // Exchange refusal
	
	/** The _party. */
	private volatile L2Party _party;
	
	// this is needed to find the inviting player for Party response
	// there can only be one active party request at once
	/** The _active requester. */
	private volatile L2PcInstance _activeRequester;
	
	/** The _request expire time. */
	private volatile long _requestExpireTime = 0;
	
	/** The _request. */
	private final L2Request _request = new L2Request(this);
	
	/** The _arrow item. */
	private volatile L2ItemInstance _arrowItem;
	
	// Used for protection after teleport
	/** The _protect end time. */
	private volatile long _protectEndTime = 0;
	
	// protects a char from agro mobs when getting up from fake death
	/** The _recent fake death end time. */
	private volatile long _recentFakeDeathEndTime = 0;
	
	/** The fists L2Weapon of the L2PcInstance (used when no weapon is equiped). */
	private volatile L2Weapon _fistsWeaponItem;
	...
	private volatile int _expertiseIndex; // index in EXPERTISE_LEVELS
	
	/** The _expertise penalty. */
	private volatile int _expertisePenalty = 0;
	
	/** The _active enchant item. */
	private volatile L2ItemInstance _activeEnchantItem = null;
	
	/** The _inventory disable. */
	protected volatile boolean _inventoryDisable = false;
	
	/** The _cubics. */
	protected L2CubicContainer _cubics = new L2CubicContainer();
	...
	private volatile SkillDat _currentSkill;
	
	/** Skills queued because a skill is already in progress. */
	private volatile SkillDat _queuedSkill;
	...
	private volatile double _cpUpdateIncCheck = .0;
	
	/** The _cp update dec check. */
	private volatile double _cpUpdateDecCheck = .0;
	
	/** The _cp update interval. */
	private volatile double _cpUpdateInterval = .0;
	
	/** The _mp update inc check. */
	private volatile double _mpUpdateIncCheck = .0;
	
	/** The _mp update dec check. */
	private volatile double _mpUpdateDecCheck = .0;
	
	/** The _mp update interval. */
	private volatile double _mpUpdateInterval = .0;
	
	/** Herbs Task Time *. */
	private int _herbstask = 0;
	
	private LgeEvent _currentLgeEvent;
	...
	public void giveRecom(final L2PcInstance target)
	{
		if (Config.ALT_RECOMMEND)
		{
			...
			{
				_log.warning("could not update char recommendations:" + e1);
			}
			finally
			{
				try
	...
	public void setKarma(int karma)
	{
		...
		if ((_karma == 0) && (karma > 0))
		{
			for (L2Object object : getKnownList().getKnownObjectArray())
			{
	...
	public void checkSSMatch(L2ItemInstance equipped, L2ItemInstance unequipped)
	{
		...
		if ((unequipped.getItem().getType2() == L2Item.TYPE2_WEAPON) && (equipped == null ? true : equipped.getItem().getCrystalType() != unequipped.getItem().getCrystalType()))
		{
			for (L2ItemInstance ss : getInventory().getItems())
			{
				...
				if ((((_itemId >= 2509) && (_itemId <= 2514)) || ((_itemId >= 3947) && (_itemId <= 3952)) || ((_itemId <= 1804) && (_itemId >= 1808)) || (_itemId == 5789) || (_itemId == 5790)
					|| (_itemId == 1835)) && (ss.getItem().getCrystalType() == unequipped.getItem().getCrystalType()))
				{
	 ...
	 * @param storeSkills
	 */
	public void giveAvailableSkills(boolean storeSkills)
	{
		int unLearnable = 0;
		int skillCounter = 0;
		...
		System.out.println("Skills: " + skills.length + " for " + this.getName());
		while (skills.length > unLearnable)
		{
			for (L2SkillLearn s : skills)
			{
				...
				{
					System.out.println(s.getId() + " cannot be learned for " + this.getName());
					unLearnable++;
					continue;
				}
		...
		if (skillCounter > 0)
		{
			sendMessage("Вы открыли " + skillCounter + " новыйх умений.");
		}
	}
	...
	public void addItem(String process, int itemId, int count, L2Object reference, boolean sendMessage)
	{
		...
		{
			// Sends message to client if requested
			if (sendMessage && ((!isCastingNow() && (ItemTable.getInstance().createDummyItem(itemId).getItemType() == L2EtcItemType.HERB))
	...
	public void setProtection(boolean protect)
	{
		if (Config.DEVELOPER && (protect || (_protectEndTime > 0)))
		{
			System.out
				.println(
					getName() + ": Protection " + (protect ? "ON " + (GameTimeController.getGameTicks() + (Config.PLAYER_SPAWN_PROTECTION * GameTimeController.TICKS_PER_SECOND)) : "OFF")
	...
	public void broadcastStatusUpdate()
	{
		...
		if (isInOlympiadMode())
		{
			...
			for (L2PcInstance player : getKnownList().getKnownPlayersArray())
			{
				if (player.getOlympiadGameId() == getOlympiadGameId())
				{
					if (Config.DEBUG)
					{
						_log
							.fine(
								"Send status for Olympia window of " + getObjectId() + "(" + getName() + ") to " + player.getObjectId() + "(" + player.getName() + "). CP: " + getCurrentCp() + " HP: "
	...
	public final void broadcastUserInfo()
	{
		...
		if (Config.DEBUG)
		{
			_log.fine("players to notify:" + getKnownList().getKnownPlayersArray().length + " packet: [S] 03 CharInfo");
		}
	 ...
	 * 
	 */
	private void sendWeaponSkillMasteryInfo()
	{
		L2Weapon weaponItem = this.getActiveWeaponItem();
		boolean mageClass = this.isMageClass();
		if ((weaponItem != null) && mageClass)
		{
			this.sendPacket(new ExPCCafePointInfo(this.getAddMagicUseSkill(), 0, true, true, 24));
		}
		else if ((weaponItem != null) && !mageClass)
		{
			this.sendPacket(new ExPCCafePointInfo(this.getAddWeaponUseSkill(weaponItem.getItemType()), 0, true, true, 24));
		}
	}
	...
	public final void broadcastTitleInfo()
	{
		...
		if (Config.DEBUG)
		{
			_log.fine("players to notify:" + getKnownList().getKnownPlayersArray().length + " packet: [S] cc TitleUpdate");
		}
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (this.isInLgeEvent())
			{
				if (Config.TVT_EVENT_TEST_MODE)
				{
					System.out.println("LgeManager.getInstance().onKill(" + killer + ", " + this + ")");
				}
				LgeManager.getInstance().onKill(killer, this);
			}
		...
		for (L2Object object : getKnownList().getKnownCharactersArray())
		{
		...
		// decrease lge point every kill
		// this.decLgePoints(1, true);
		return true;
	}
	...
	public void updateOnlineStatus()
	{
		...
		{
			_log.warning("could not set char online status:" + e1);
		}
		finally
		{
			try
	...
	private boolean createDb()
	{
		...
		try
		{
			...
			statement = con
				.prepareStatement(
					"INSERT INTO characters " + "(account_name,obj_Id,char_name,level,maxHp,curHp,maxCp,curCp,maxMp,curMp," + "acc,crit,evasion,mAtk,mDef,mSpd,pAtk,pDef,pSpd,runSpd,walkSpd,"
						+ "str,con,dex,_int,men,wit,face,hairStyle,hairColor,sex," + "movement_multiplier,attack_speed_multiplier,colRad,colHeight,"
						+ "exp,sp,karma,pvpkills,pkkills,clanid,maxload,race,classid,deletetime," + "cancraft,title,accesslevel,online,isin7sdungeon,clan_privs,wantspeace,"
						+ "base_class,newbie,nobless,power_grade,last_recom_date,_add_str,_add_dex,_add_con,_add_wit,_add_men,_add_int) "
						+ "values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	private static L2PcInstance restore(int objectId)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				player.setAddSTR(rset.getInt("_add_str"));
				player.setAddDEX(rset.getInt("_add_dex"));
				player.setAddCON(rset.getInt("_add_con"));
				player.setAddWIT(rset.getInt("_add_wit"));
				player.setAddMEN(rset.getInt("_add_men"));
				player.setAddINT(rset.getInt("_add_int"));
	...
	private void setAddINT(int addint)
	{
		this._addINT = addint;
		this._addINT = Math.min(this._addINT, 99);
	}
	
	private void setAddWIT(int addwit)
	{
		this._addWIT = addwit;
		this._addWIT = Math.min(this._addWIT, 99);
	}
	
	private void setAddMEN(int addmen)
	{
		this._addMEN = addmen;
		this._addMEN = Math.min(this._addMEN, 99);
	}
	
	private void setAddCON(int addcon)
	{
		this._addCON = addcon;
		this._addCON = Math.min(this._addCON, 99);
	}
	
	private void setAddSTR(int addstr)
	{
		this._addSTR = addstr;
		this._addSTR = Math.min(this._addSTR, 99);
	}
	
	private void setAddDEX(int adddex)
	{
		this._addDEX = adddex;
		this._addDEX = Math.min(this._addDEX, 99);
	}
	
	// add functions
	public void AddINT(int addint)
	{
		this._addINT += addint;
		this._addINT = Math.min(this._addINT, 99);
	}
	
	public void AddWIT(int addwit)
	{
		this._addWIT += addwit;
		this._addWIT = Math.min(this._addWIT, 99);
	}
	
	public void AddMEN(int addmen)
	{
		this._addMEN += addmen;
		this._addMEN = Math.min(this._addMEN, 99);
	}
	
	public void AddCON(int addcon)
	{
		this._addCON += addcon;
		this._addCON = Math.min(this._addCON, 99);
	}
	...
	private void storeCharBase()
	{
		...
		try
		{
			...
			statement.setInt(57, _addSTR);
			statement.setInt(58, _addDEX);
			statement.setInt(59, _addCON);
			statement.setInt(60, _addWIT);
			statement.setInt(61, _addMEN);
			statement.setInt(62, _addINT);
	...
	private void storeCharSub()
	{
		...
		{
			_log.warning("Could not store sub class data for " + getName() + ": " + e1);
		}
		finally
		{
			try
	...
	private void storeEffect()
	{
		...
		try
		{
			...
			for (L2Effect effect : getAllEffects())
			{
				if ((effect != null) && effect.getInUse() && !effect.getSkill().isToggle())
				{
					...
					{
						TimeStamp t1 = ReuseTimeStamps.remove(skillId);
						statement.setLong(6, t1.hasNotPassed() ? t1.getReuse() : 0);
					}
					else
					{
			...
			for (TimeStamp t2 : ReuseTimeStamps.values())
			{
				if (t2.hasNotPassed())
				{
					buff_index++;
					statement = con.prepareStatement(ADD_SKILL_SAVE);
					statement.setInt(1, getObjectId());
					statement.setInt(2, t2.getSkill());
		...
		{
			_log.warning("Could not store char effect data: " + e1);
		}
		finally
		{
			try
	...
	public L2Skill removeSkill(L2Skill skill)
	{
		...
		{
			_log.warning("Error could not delete skill: " + e1);
		}
		finally
		{
			try
		...
		if (attacker instanceof L2PcInstance)
		{
			...
			if (getClan() != null)
			{
				...
				if ((getClan() != null) && (((L2PcInstance) attacker).getClan() != null)
					&& (getClan().isAtWarWith(((L2PcInstance) attacker).getClanId()) && (getWantsPeace() == 0) && (((L2PcInstance) attacker).getWantsPeace() == 0) && !isAcademyMember()))
				{
					return true;
				}
			}
	...
	public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (skill.isOffensive())
		{
			...
			if (!target.isAutoAttackable(this) && !forceUse && (sklTargetType != SkillTargetType.TARGET_AURA) && (sklTargetType != SkillTargetType.TARGET_CLAN)
				&& (sklTargetType != SkillTargetType.TARGET_ALLY) && (sklTargetType != SkillTargetType.TARGET_PARTY) && (sklTargetType != SkillTargetType.TARGET_SELF))
			{
				// Send a Server->Client packet ActionFailed to the L2PcInstance
				sendPacket(new ActionFailed());
				return;
		...
		if (!skill.isOffensive())
		{
			...
			if ((target instanceof L2MonsterInstance) && !forceUse && (sklTargetType != SkillTargetType.TARGET_PET) && (sklTargetType != SkillTargetType.TARGET_AURA)
				&& (sklTargetType != SkillTargetType.TARGET_CLAN) && (sklTargetType != SkillTargetType.TARGET_SELF) && (sklTargetType != SkillTargetType.TARGET_PARTY)
				&& (sklTargetType != SkillTargetType.TARGET_ALLY) && (sklTargetType != SkillTargetType.TARGET_CORPSE_MOB) && (sklTargetType != SkillTargetType.TARGET_AREA_CORPSE_MOB)
	...
	public boolean checkLandingState()
	{
		...
		if (isInsideZone(ZONE_SIEGE) && !((getClan() != null) && (CastleManager.getInstance().getCastle(this) == CastleManager.getInstance().getCastleByOwner(getClan()))
	 ...
	 * @return the cubics
	 */
	public L2CubicContainer getCubics()
	{
		return _cubics;
	}
		...
		for (int itemId : _activeSoulShots.values())
		{
			...
			if (item != null)
			{
				if (magic)
				{
					if (!summon)
					{
						if ((itemId == 2509) || (itemId == 2510) || (itemId == 2511) || (itemId == 2512) || (itemId == 2513) || (itemId == 2514) || (itemId == 3947) || (itemId == 3948)
							|| (itemId == 3949) || (itemId == 3950) || (itemId == 3951) || (itemId == 3952) || (itemId == 5790))
						{
	...
	public void restartRecom()
	{
		if (Config.ALT_RECOMMEND)
		{
			...
			{
				_log.warning("could not clear char recommendations: " + e1);
			}
			finally
			{
				try
	...
	public void doRevive()
	{
		...
		// clear next spawn locations
		clearNextSpawnLocation();
	}
		...
		if (!isDead() && (_taskforfish == null))
		{
			...
			if (_lure != null)
			{
				...
				if ((lureid == 6519) || (lureid == 6522) || (lureid == 6525) || (lureid == 8505) || (lureid == 8508) || (lureid == 8511))
				{
					...
					|| ((lureid >= 7807) && (lureid <= 7809)) || ((lureid >= 8484) && (lureid <= 8486)))
				{
			...
			_taskforfish =
				ThreadPoolManager.getInstance().scheduleEffectAtFixedRate(new LookingForFishTask(_fish.getWaitTime(), _fish.getFishGuts(), _fish.getType(), isNoob, isUpperGrade), 10000, checkDelay);
		}
	}
	...
	{
		if ((Rnd.get(100) <= Config.DEATH_PENALTY_CHANCE) && !(killer instanceof L2PcInstance) && !(isGM())
			&& !(getCharmOfLuck() && ((killer instanceof L2BossInstance) || (killer instanceof L2RaidBossInstance))))
		{
	...
	private final HashMap<Integer, TimeStamp> ReuseTimeStamps = new HashMap<>();
	private int _lge_points;
	...
	public final int getAddWIT()
	{
		return this._addWIT;
	}
	
	public final int getAddMEN()
	{
		return this._addMEN;
	}
	...
	public int GetCurrentAlternativeHennas()
	{
		return _addCON + _addMEN + _addWIT + _addINT + _addDEX + _addSTR;
	}
	
	public boolean AddAlternativeHenna(String HennaType, int addCount)
	{
		int currentHennas = GetCurrentAlternativeHennas();
		if ((currentHennas + addCount) > this.getLevel())
		{
			return false;
		}
		switch (HennaType)
		{
			case "con":
				_addCON = _addCON + addCount;
				break;
			case "men":
				_addMEN = _addMEN + addCount;
				break;
			case "wit":
				_addWIT = _addWIT + addCount;
				break;
			case "int":
				_addINT = _addINT + addCount;
				break;
			case "dex":
				_addDEX = _addDEX + addCount;
				break;
			case "str":
				_addSTR = _addSTR + addCount;
				break;
			default:
				break;
		}
	 ...
	 * @return
	 */
	public int getFreeAlternativeHennas()
	{
		return this.getLevel() - GetCurrentAlternativeHennas();
	}
	
	/**
	 * @return the _currentTvTEvent
	 */
	public LgeEvent getCurrentLgeEvent()
	{
		return _currentLgeEvent;
	}
	
	/**
	 * @param _currentTvTEvent the _currentTvTEvent to set
	 */
	public void setCurrentLgeEvent(LgeEvent _currentTvTEvent)
	{
		this._currentLgeEvent = _currentTvTEvent;
	}
	
	/**
	 * @return
	 */
	public boolean isInLgeEvent()
	{
		return (this._currentLgeEvent != null) && !this._currentLgeEvent.isInactivating() && !this._currentLgeEvent.isInactive();
	}
	...
	/**
	 * @return the _lge_points
	 */
	public int getLgePoints()
	{
		return _lge_points;
	}
	
	/**
	 * @param _lge_points the _lge_points to set
	 * @param store
	 */
	public void incLgePoints(int _lge_points, boolean store)
	{
		this._lge_points += _lge_points;
		
		if (store)
		{
			this.sendPacket(new ExPCCafePointInfo(this._lge_points, _lge_points, true, false, 24));
		}
	}
	
	public boolean decLgePoints(int _lge_points, boolean store)
	{
		if ((this._lge_points - _lge_points) < 0)
		{
			return false;
		}
		...
		if (store)
		{
			this.sendPacket(new ExPCCafePointInfo(this._lge_points, _lge_points, false, false, 24));
		}
	 ...
	 * @param z
	 */
	public void setNextSpawnLocation(int x, int y, int z)
	{
		this._nextSpawnLocation = new int[]
		{
			x,
			y,
			z
	...
	public void clearNextSpawnLocation()
	{
		this._nextSpawnLocation = null;
	}
	
	public Location getNextSpawnLocation()
	{
		if (this._nextSpawnLocation != null)
		{
			return new Location(this._nextSpawnLocation[0], this._nextSpawnLocation[1], this._nextSpawnLocation[2]);
		}
		return null;
	}
	 ...
	 * @param smID
	 */
	public void sendSystemMessage(SystemMessageId smID)
	{
		this.sendPacket(new SystemMessage(smID));
	}
	
	private short getSkillsCountFromDataBase()
	{
		java.sql.Connection con = null;
		short skillsCount = 0;
		try
	...
	@Override
	public short getAveableSkills()
	{
		return (short) Math.max(getMaximumSkillsCount() - getSkillsCountFromDataBase(), 0);
	}
	
	public short getMaximumSkillsCount()
	{
		// 20 / 3 = 7
		// 80 / 3 = 27
		return (short) (this.getLevel() / 3);
	}
	...
	/*
	 * (non-Javadoc)
	 * @see net.sf.l2j.gameserver.model.L2Character#addWeaponUseSkill(net.sf.l2j.gameserver.templates.L2WeaponType)
	 */
	@Override
	protected void addWeaponUseSkill(L2WeaponType itemType)
	{
		if ((itemType == L2WeaponType.BOW) || (itemType == L2WeaponType.BIGBLUNT) || (itemType == L2WeaponType.BIGSWORD))
		{
			_addWeaponUseSkill[itemType.ordinal()] += Rnd.get(10) + 1;
		}
		else
		{
			_addWeaponUseSkill[itemType.ordinal()]++;
		}
	}
	 ...
	 */
	@Override
	protected void addMagicUseSkill()
	{
		_addMagicUseSkill += Rnd.get(10) + 1;
	}
	
	/*
	 * (non-Javadoc)
	 * @see net.sf.l2j.gameserver.model.L2Character#getMagicUseSkill()
	 */
	@Override
	public int getAddMagicUseSkill()
	{
		return _addMagicUseSkill;
	}
	
	/*
	 * (non-Javadoc)
	 * @see net.sf.l2j.gameserver.model.L2Character#getAddWeaponUseSkill(net.sf.l2j.gameserver.templates.L2WeaponType)
	 */
	@Override
	public int getAddWeaponUseSkill(L2WeaponType itemType)
	{
		return _addWeaponUseSkill[itemType.ordinal()];
	}
