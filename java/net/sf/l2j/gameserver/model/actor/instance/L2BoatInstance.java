...
public class L2BoatInstance extends L2Character
{
	...
	private class L2BoatTrajet
	{
		 ...
		 * @param pBoatname
		 */
		public L2BoatTrajet(int pIdWaypoint1, int pIdWTicket1, int pNtx1, int pNty1, int pNtz1, String pNpc1, String pSysmess10_1, String pSysmess5_1, String pSysmess1_1, String pSysmess0_1,
		...
		public int state(int state, L2BoatInstance _boat)
		{
			if (state < max)
			{
				...
				L2PcInstance[] knownPlayers = _boat.getKnownList().getKnownPlayersArray();
				if ((knownPlayers == null) || (knownPlayers.length == 0))
				{
					return bp.time;
				}
	...
	public void begin()
	{
		if (_cycle == 1)
		{
			L2PcInstance[] knownPlayers = getKnownList().getKnownPlayersArray();
			if ((knownPlayers != null) && (knownPlayers.length > 0))
			{
				_inboat = new HashMap<>();
				int i = 0;
