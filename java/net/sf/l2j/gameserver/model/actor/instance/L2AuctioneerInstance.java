		...
		else if (condition == COND_REGULAR)
		{
			...
			if (actualCommand.equalsIgnoreCase("auction"))
			{
				...
				try
				{
					...
					try
					{
						...
						Auction a =
							new Auction(player.getClan().getHasHideout(), player.getClan(), days * 86400000L, bid, ClanHallManager.getInstance().getClanHallByOwner(player.getClan()).getName());
			...
			else if (actualCommand.equalsIgnoreCase("list"))
			{
				...
				for (Auction a : auctions)
				{
					...
					items += "<tr>" + "<td>" + ClanHallManager.getInstance().getClanHallById(a.getItemId()).getLocation() + "</td>" + "<td><a action=\"bypass -h npc_" + getObjectId() + "_bidding "
						+ a.getId() + "\">" + a.getItemName() + "</a></td>" + "<td>" + format.format(a.getEndDate()) + "</td>" + "<td>" + a.getStartingBid() + "</td>" + "</tr>";
