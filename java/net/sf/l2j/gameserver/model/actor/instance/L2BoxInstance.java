...
public class L2BoxInstance extends L2NpcInstance
{
	...
	{/*
		 ...
		 * statement.setString(5, item.getItem().getName()); statement.setInt(6, item.getCount()); statement.setInt(7, item.getEnchantLevel()); statement.execute(); statement.close(); } else { PreparedStatement statement = con.prepareStatement("UPDATE boxes SET count=? WHERE id=?");
		 * statement.setInt(1, foundCount + item.getCount()); statement.setInt(2, foundId); statement.execute(); statement.close(); } } catch (Exception e) { _log.info("could not store item to box "+getSpawn().getId()+"-"+drawer+" for char "+charname); } finally { try { con.close(); } catch
		 * (Exception e) {} }
		 */
	}
	...
	{/*
		 ...
		 * item.setCount(bi.count); item.setEnchantLevel(bi.enchant); player.getInventory().addItem("Box", item, player, this); } } catch (Exception e) { _log.fine("takeOutBox "+command+" failed: "+e); } } } private L2BoxItem takeItemOutBox(L2PcInstance player, String drawer, L2ItemInstance item) {
