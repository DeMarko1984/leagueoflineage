...
public class L2NpcInstance extends L2Character
{
	...
	private final boolean _isInTown = false;
	...
	public void onActionShift(L2GameClient client)
	{
		...
		if (player.getAccessLevel() >= Config.GM_ACCESSLEVEL)
		{
			...
			html1
				.append(
					"<tr><td>Max.HP</td><td>" + (int) (getMaxHp() / getStat().calcStat(Stats.MAX_HP, 1, this, null)) + "*" + getStat().calcStat(Stats.MAX_HP, 1, this, null)
			...
			html1
				.append(
					"<br><center><table><tr><td><button value=\"Edit NPC\" action=\"bypass -h admin_edit_npc " + getTemplate().npcId
			...
			html1
				.append(
					"<tr><td><button value=\"Show DropList\" action=\"bypass -h admin_show_droplist " + getTemplate().npcId
		...
		else if (Config.ALT_GAME_VIEWNPC)
		{
			...
			html1
				.append(
					"<tr><td>Max.HP</td><td>" + (int) (getMaxHp() / getStat().calcStat(Stats.MAX_HP, 1, this, null)) + "*" + (int) getStat().calcStat(Stats.MAX_HP, 1, this, null)
						+ "</td><td>Max.MP</td><td>" + getMaxMp() + "</td></tr>");
			html1.append("<tr><td>P.Atk.</td><td>" + getPAtk(null) + "</td><td>M.Atk.</td><td>" + getMAtk(null, null) + "</td></tr>");
			html1.append("<tr><td>P.Def.</td><td>" + getPDef(null) + "</td><td>M.Def.</td><td>" + getMDef(null, null) + "</td></tr>");
			html1.append("<tr><td>Accuracy</td><td>" + getAccuracy() + "</td><td>Evasion</td><td>" + getEvasionRate(null) + "</td></tr>");
	...
	public final Castle getCastle()
	{
		...
		_castleIndex = CastleManager.getInstance().getCastleIndex(CastleManager.getInstance().getCastle("Aden").getCastleId());
	...
	public void onBypassFeedback(L2PcInstance player, String command)
	{
		// if (canInteract(player))
		{
			...
			{
				makeHPRecovery(player);
			}
			else if (command.startsWith("MPRecovery"))
			{
				makeMPRecovery(player);
			}
			else if (command.startsWith("SupportMagic"))
			{
				makeSupportMagic(player);
			}
			else if (command.startsWith("WarriorBuff"))
			{
				makeWarriorBuff(player, command);
			}
			else if (command.startsWith("MageBuff"))
			{
				makeMageBuff(player, command);
			}
			...
			else if (command.startsWith("AddHennaSTR"))
			{
				if (player.AddAlternativeHenna("str", 1))
				{
					player.store();
			...
			else if (command.startsWith("AddHennaCON"))
			{
				if (player.AddAlternativeHenna("con", 1))
				{
					player.store();
					player.sendPacket(new UserInfo(player));
				}
			...
			else if (command.startsWith("AddHennaMEN"))
			{
				if (player.AddAlternativeHenna("men", 1))
				{
					player.store();
					player.sendPacket(new UserInfo(player));
				}
			...
			else if (command.startsWith("AddHennaDEX"))
			{
				if (player.AddAlternativeHenna("dex", 1))
				{
					player.store();
					player.sendPacket(new UserInfo(player));
				}
			...
			else if (command.startsWith("AddHennaWIT"))
			{
				if (player.AddAlternativeHenna("wit", 1))
				{
					player.store();
					player.sendPacket(new UserInfo(player));
				}
			...
			else if (command.startsWith("AddHennaINT"))
			{
				if (player.AddAlternativeHenna("int", 1))
				{
					player.store();
					player.sendPacket(new UserInfo(player));
				}
	 ...
	 * @param command
	 */
	private void makeMageBuff(L2PcInstance player, String command)
	{
		int[] _skillIds = new int[]
		{
			1085,
			1059,
			1078,
		...
		L2Character actor = player;
		if (command.startsWith("MageBuffPet"))
		{
			actor = player.getPet();
		}
		...
		int moneyToReduce = (player.getLevel() >= 40 ? player.getLevel() * 1000 : 0);
		
		if ((player.getLevel() >= 40) && !player.reduceAdena("WarriorBuff", moneyToReduce, player.getLastFolkNPC(), true))
		{
			player.sendSystemMessage(SystemMessageId.YOU_NOT_ENOUGH_ADENA);
			player.sendActionFailed();
			return;
		}
		
		for (int skid : _skillIds)
		{
			// stop skill effects
			actor.stopSkillEffects(skid);
			// give skill effects
			L2Effect[] effects = SkillTable.getInstance().getInfo(skid, SkillTable.getInstance().getMaxLevel(skid, 1)).getEffects(actor, actor);
	 ...
	 * @param command
	 */
	private void makeWarriorBuff(L2PcInstance player, String command)
	{
		int[] _skillIds = new int[]
		{
			1068,
		...
		L2Character actor = player;
		if (command.startsWith("WarriorBuffPet"))
		{
			actor = player.getPet();
		}
	...
	public void makeCPRecovery(L2PcInstance player)
	{
		...
		int neededmoney = (int) (player.getMaxCp() - player.getCurrentCp());
		if (neededmoney == 0)
		{
			player.sendActionFailed();
			return;
		...
		sm.addString(String.valueOf(neededmoney));
		player.sendPacket(sm);
	}
	
	public void makeHPRecovery(L2PcInstance player)
	{
		if (player.isCursedWeaponEquiped())
		{
			player.sendMessage("Go away, you're not welcome here.");
		...
		int neededmoney = (int) (player.getMaxHp() - player.getCurrentHp());
		if (neededmoney == 0)
		{
			player.sendActionFailed();
			return;
		...
		SystemMessage sm;
		if (!player.reduceAdena("RestoreHP", neededmoney, player.getLastFolkNPC(), true))
		{
			player.sendSystemMessage(SystemMessageId.YOU_NOT_ENOUGH_ADENA);
			return;
		}
	...
	public void makeMPRecovery(L2PcInstance player)
	{
		if (player.isCursedWeaponEquiped())
		{
			player.sendMessage("Go away, you're not welcome here.");
		...
		int neededmoney = (int) (player.getMaxMp() - player.getCurrentMp());
		if (neededmoney == 0)
		{
			player.sendActionFailed();
			return;
		...
		SystemMessage sm;
		if (!player.reduceAdena("RestoreMP", neededmoney, player.getLastFolkNPC(), true))
		{
			player.sendSystemMessage(SystemMessageId.YOU_NOT_ENOUGH_ADENA);
			return;
		}
	...
	public void makeSupportMagic(L2PcInstance player)
	{
		...
		// {
		// String content = "<html><body>Newbie Guide:<br>Only a <font color=\"LEVEL\">novice character of level " + higestLevel
	...
	public void showChatWindow(L2PcInstance player, int val)
	{
		...
		html.replace("%objectId%", String.valueOf(getObjectId()));
		html.replace("%hennapoints%", String.valueOf(player.getFreeAlternativeHennas()));
	...
	public void onDecay()
	{
		...
		if (this instanceof L2LgeTowerInstance)
		{
			((L2LgeTowerInstance) this).onDeath();
		}
