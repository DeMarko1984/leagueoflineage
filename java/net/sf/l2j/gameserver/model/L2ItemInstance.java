...
public final class L2ItemInstance extends L2Object
{
	...
	public void onAction(L2PcInstance player)
	{
		...
		if (((_itemId >= 3960) && (_itemId <= 4021) && player.isInParty()) || ((_itemId >= 3960) && (_itemId <= 3969) && !player.isCastleLord(1))
			|| ((_itemId >= 3973) && (_itemId <= 3982) && !player.isCastleLord(2)) || ((_itemId >= 3986) && (_itemId <= 3995) && !player.isCastleLord(3))
			|| ((_itemId >= 3999) && (_itemId <= 4008) && !player.isCastleLord(4)) || ((_itemId >= 4012) && (_itemId <= 4021) && !player.isCastleLord(5))
			|| ((_itemId >= 5205) && (_itemId <= 5214) && !player.isCastleLord(6)) || ((_itemId >= 6779) && (_itemId <= 6788) && !player.isCastleLord(7))
	...
	public static L2ItemInstance restoreFromDb(int objectId)
	{
		...
		try
		{
			...
			PreparedStatement statement = con
				.prepareStatement(
					"SELECT owner_id, object_id, item_id, count, enchant_level, loc, loc_data, price_sell, price_buy, custom_type1, custom_type2, mana_left FROM items WHERE object_id = ?");
	...
	{
		ObjectPosition position = getPosition();
		L2WorldRegion worldRegion = position.getWorldRegion();
		if (Config.ASSERT)
		{
			assert worldRegion == null;
		}
		
		L2World instance = L2World.getInstance();
		synchronized (this)
		{
			// Set the x,y,z position of the L2ItemInstance dropped and update its _worldregion
			setIsVisible(true);
			position.setWorldPosition(x, y, z);
			worldRegion = instance.getRegion(position.getWorldPosition());
			position.setWorldRegion(worldRegion);
			
			// Add the L2ItemInstance dropped to _visibleObjects of its L2WorldRegion
			worldRegion.addVisibleObject(this);
			
			// change instance id
			setInstanceId(dropper.getInstanceId());
		}
		...
		instance.addVisibleObject(this, worldRegion, dropper);
		
		// save dropped items manager
		...
		ThreadPoolManager.getInstance().executeTask(() ->
		{
			...
			try
			{
				...
				PreparedStatement statement =
					con.prepareStatement("UPDATE items SET owner_id=?,count=?,loc=?,loc_data=?,enchant_level=?,price_sell=?,price_buy=?,custom_type1=?,custom_type2=?,mana_left=? WHERE object_id = ?");
			...
			{
				_log.log(Level.SEVERE, e1.getMessage());
			}
			finally
			{
				try
	...
	private void insertIntoDb()
	{
		...
		try
		{
			...
			PreparedStatement statement = con
				.prepareStatement(
					"INSERT INTO items (owner_id,item_id,count,loc,loc_data,enchant_level,price_sell,price_buy,object_id,custom_type1,custom_type2,mana_left) " + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
			statement.setInt(1, _ownerId);
