...
public class Duel
{
	...
	public void restorePlayerConditions(boolean abnormalDuelEnd)
	{
		...
		// restore player conditions
		for (Object element : _playerConditions)
		{
			PlayerCondition playerCondition = (PlayerCondition) element;
			playerCondition.restoreCondition();
		}
	}
	...
	public DuelResultEnum checkEndDuelCondition()
	{
		...
		else if (!_partyDuel)
		{
			...
			if (_playerA.isInsideZone(L2Character.ZONE_PEACE) || _playerB.isInsideZone(L2Character.ZONE_PEACE) || _playerA.isInsideZone(L2Character.ZONE_SIEGE)
				|| _playerB.isInsideZone(L2Character.ZONE_SIEGE) || _playerA.isInsideZone(L2Character.ZONE_PVP) || _playerB.isInsideZone(L2Character.ZONE_PVP))
			{
				return DuelResultEnum.Canceled;
			}
		}
	...
	public void onRemoveFromParty(L2PcInstance player)
	{
		...
		if ((player == _playerA) || (player == _playerB))
		{
			...
			{
				PlayerCondition playerCondition = (PlayerCondition) element;
				playerCondition.teleportBack();
				playerCondition.getPlayer().setIsInDuel(0);
			}
		...
		// teleport the player back & delete his PlayerCondition record
		{
			...
			{
				PlayerCondition playerCondition = (PlayerCondition) element;
				if (playerCondition.getPlayer() == player)
				{
					playerCondition.teleportBack();
					_playerConditions.remove(playerCondition);
					break;
				}
			}
	...
	public void onBuff(L2PcInstance player, L2Effect debuff)
	{
		for (Object element : _playerConditions)
		{
			...
			if (playerCondition.getPlayer() == player)
			{
				playerCondition.registerDebuff(debuff);
				return;
			}
		}
	}
