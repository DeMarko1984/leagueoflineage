...
import net.sf.l2j.gameserver.skills.l2skills.L2SkillTeleportTo;
...
public abstract class L2Skill
{
	...
	public static enum SkillType
	{
		...
		BEAST_FEED,
		FORCE_BUFF,
		TELEPORT_TO(L2SkillTeleportTo.class),
	...
	public boolean checkCondition(L2Character activeChar, L2Object target, boolean itemOrWeapon)
	{
		...
		Env env = new Env();
		env.player = new WeakReference<>(activeChar);
		if (target instanceof L2Character)
		{
			env.target = new WeakReference<>((L2Character) target);
		}
		env.skill = this;
	...
	public final L2Object[] getTargetList(L2Character activeChar, boolean onlyFirst)
	{
		...
		switch (targetType)
		{
			...
			case TARGET_AURA:
			{
				...
				for (L2Object obj : activeChar.getKnownList().getKnownObjectArray())
				{
			...
			case TARGET_ALLY:
			{
				if (activeChar instanceof L2PcInstance)
				{
					...
					if (clan != null)
					{
						// Get all visible objects in a spheric area near the L2Character
						// Get Clan Members
						for (L2Object newTarget : activeChar.getKnownList().getKnownObjectArray())
						{
			...
			case TARGET_AREA_UNDEAD:
			{
				...
				{
					for (L2Object obj : cha.getKnownList().getKnownObjectArray())
					{
						if (obj == null)
						{
							continue;
	...
	public final Func[] getStatFuncs(L2Effect effect, L2Character player)
	{
		...
		{
			Env env = new Env();
			env.player = new WeakReference<>(player);
			env.skill = this;
	...
	public final L2Effect[] getEffects(L2Character effector, L2Character effected)
	{
		...
		{
			Env env = new Env();
			env.player = new WeakReference<>(effector);
			env.target = new WeakReference<>(effected);
			env.skill = this;
	...
	public final L2Effect[] getEffectsSelf(L2Character effector)
	{
		...
		for (EffectTemplate et : _effectTemplatesSelf)
		{
			Env env = new Env();
			env.player = new WeakReference<>(effector);
			env.target = new WeakReference<>(effector);
			env.skill = this;
			L2Effect e = et.getEffect(env);
			if (e != null)
			{
				...
				if (e.getEffectType() == L2Effect.EffectType.CHARGE)
				{
					...
					EffectCharge effect = (EffectCharge) env.target.get().getFirstEffect(L2Effect.EffectType.CHARGE);
					if (effect != null)
					{
						...
						if (effectcharge < _numCharges)
						{
							...
							if (env.isNoWeakTarget() && (env.target.get() instanceof L2PcInstance))
							{
								env.target.get().sendPacket(new EtcStatusUpdate((L2PcInstance) env.target.get()));
								SystemMessage sm = new SystemMessage(SystemMessageId.FORCE_INCREASED_TO_S1);
								sm.addNumber(effectcharge);
								env.target.get().sendPacket(sm);
							}
						}
					}
					else
