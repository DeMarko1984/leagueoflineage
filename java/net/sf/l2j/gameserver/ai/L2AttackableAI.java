...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target instanceof L2PcInstance)
		{
			...
			if (target.isInParty() && party.isInDimensionalRift())
			{
				DimensionalRift dimensionalRift = party.getDimensionalRift();
				byte riftType = dimensionalRift.getType();
				byte riftRoom = dimensionalRift.getCurrentRoom();
		...
		boolean canSeeTarget = GeoData.getInstance().canSeeTarget(me, target);
		if (_actor instanceof L2GuardInstance)
		{
			// Check if the L2PcInstance target has karma (=PK)
			// if ((target instanceof L2PcInstance) && (((L2PcInstance) target).getKarma() > 0))
			if ((target instanceof L2PcInstance) || (target instanceof L2SummonInstance))
			{
				// Los Check
				return canSeeTarget;
			}
		...
		{ // The actor is a L2MonsterInstance
			...
			return (me.isAggressive() && canSeeTarget);
		}
	}
	...
	synchronized void changeIntention(CtrlIntention intention, Object arg0, Object arg1)
	{
		if ((intention == AI_INTENTION_IDLE) || (intention == AI_INTENTION_ACTIVE))
		{
			...
			if (!_actor.isAlikeDead())
			{
				...
				if (npc.getKnownList().getKnownPlayersArray().length > 0)
				{
	...
	private void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			L2Object[] knownCharactersArray = npc.getKnownList().getKnownPlayableArray();
			for (L2Object obj : knownCharactersArray)
			{
	...
	private void thinkAttack()
	{
		...
		else
		{
			...
			if (((L2NpcInstance) _actor).getFactionId() != null)
			{
				...
				for (L2Object obj : _actor.getKnownList().getKnownNpcInstanceArray())
				{
					if (obj instanceof L2NpcInstance)
					{
						...
						if (_actor.isInsideRadius(npc, npc.getFactionRange(), true, false) && GeoData.getInstance().canSeeTarget(_actor, npc) && (Math.abs(getAttackTarget().getZ() - npc.getZ()) < 600)
							&& (npc.getAI() != null) && _actor.getAttackByList().contains(getAttackTarget())
				...
				if (!_actor.isMuted() && (!Config.ALT_GAME_MOB_ATTACK_AI || ((_actor instanceof L2MonsterInstance) && (Rnd.nextInt(100) <= 5))))
				{
					for (L2Skill sk : skills)
					{
						...
						if ((((sk.getSkillType() == L2Skill.SkillType.BUFF) || (sk.getSkillType() == L2Skill.SkillType.HEAL))
							|| ((dist2 >= ((castRange * castRange) / 9.0)) && (dist2 <= (castRange * castRange)) && (castRange > 70))) && !_actor.isSkillDisabled(sk.getId())
							&& (_actor.getCurrentMp() >= _actor.getStat().getMpConsume(sk)) && !sk.isPassive() && (Rnd.nextInt(100) <= 5))
						{
			...
			else
			{
				...
				if (!_actor.isMuted() /* && _rnd.nextInt(100) <= 5 */)
				{
					...
					for (L2Skill sk : skills)
					{
						if (/* sk.getCastRange() >= dist && sk.getCastRange() <= 70 && */!sk.isPassive() && (_actor.getCurrentMp() >= _actor.getStat().getMpConsume(sk))
							&& !_actor.isSkillDisabled(sk.getId()) && ((Rnd.nextInt(100) <= 8) || ((_actor instanceof L2PenaltyMonsterInstance) && (Rnd.nextInt(100) <= 20))))
						{
							L2Object OldTarget = _actor.getTarget();
							if ((sk.getSkillType() == L2Skill.SkillType.BUFF) || (sk.getSkillType() == L2Skill.SkillType.HEAL))
							{
