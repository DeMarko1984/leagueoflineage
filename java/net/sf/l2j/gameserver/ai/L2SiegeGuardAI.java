			...
			if (!Config.ALT_GAME_MOB_ATTACK_AI || ((_actor instanceof L2MonsterInstance) && (Rnd.nextInt(100) <= 5)))
			{
				for (L2Skill sk : skills)
				{
					...
					if ((((sk.getSkillType() == L2Skill.SkillType.BUFF) || (sk.getSkillType() == L2Skill.SkillType.HEAL))
						|| ((dist_2 >= ((castRange * castRange) / 9)) && (dist_2 <= (castRange * castRange)) && (castRange > 70))) && !_actor.isSkillDisabled(sk.getId())
		...
		else if (dist_2 <= ((range + 20) * (range + 20)))
		{
			...
			if (!_actor.isMuted() && (Rnd.nextInt(100) <= 5))
			{
				for (L2Skill sk : skills)
				{
					...
					if (((castRange * castRange) >= dist_2) && (castRange <= 70) && !sk.isPassive() && (_actor.getCurrentMp() >= _actor.getStat().getMpConsume(sk))
	...
	private final void factionNotify()
	{
		...
		for (L2Character cha : _actor.getKnownList().getKnownCharactersInRadius(1000))
		{
			...
			if (((npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE) || (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE))
				&& _actor.isInsideRadius(npc, npc.getFactionRange(), false, true) && (npc.getTarget() == null) && _attackTarget.isInsideRadius(npc, npc.getFactionRange(), false, true))
			{
				if (Config.GEODATA > 0)
				{
