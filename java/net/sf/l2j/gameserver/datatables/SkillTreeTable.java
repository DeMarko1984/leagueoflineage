...
public class SkillTreeTable
{
	...
	public L2SkillLearn[] getAvailableSkillsAllClasses(L2PcInstance cha, ClassId chaClassId)
	{
		return getAvailableSkillsAllClasses(cha, chaClassId, false);
	}
	
	public L2SkillLearn[] getAvailableSkillsAllClasses(L2PcInstance cha, ClassId chaClassId, boolean onlyKnown)
	{
		List<L2SkillLearn> result = new ArrayList<>();
		
		// generate classes array
		List<ClassId> _classes = new ArrayList<>();
		
		for (ClassId cId : ClassId.values())
		{
			if ((cId.level() == 3) && (chaClassId.isMage() == cId.isMage()) && (chaClassId.getRace() == cId.getRace()))
			{
				_classes.add(cId);
			}
		}
		
		if (chaClassId == ClassId.dwarvenFighter)
		{
			_classes.add(ClassId.titan);
		}
		
		// generate full array of all classes skills
		// need remove dublicates
		List<L2SkillLearn> _allskills = new ArrayList<>();
		for (ClassId classId : _classes)
		{
			...
			{
				if ((temp.getMinLevel() <= cha.getLevel()) && !_allskills.contains(temp))
				{
					_allskills.add(temp);
				}
			}
		...
		for (L2SkillLearn temp : _allskills)
		{
			if (temp.getMinLevel() <= cha.getLevel())
			{
				...
				if (!knownSkill && (temp.getLevel() == 1) && !onlyKnown)
				{
					// this is a new skill
					result.add(temp);
				}
	...
	{
		return getAvailableSkills(cha, false);
	}
	
	public L2SkillLearn[] getAvailableSkills(L2PcInstance cha, boolean onlyKnown)
	{
		List<L2SkillLearn> result = new ArrayList<>();
