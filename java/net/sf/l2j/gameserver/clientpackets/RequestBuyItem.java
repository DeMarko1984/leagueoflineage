...
public final class RequestBuyItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		) || !((target instanceof L2MerchantInstance) || (target instanceof L2FishermanInstance) || (target instanceof L2MercManagerInstance) || (target instanceof L2ClanHallManagerInstance)
		...
		for (int i = 0; i < _count; i++)
		{
			...
			if ((count > Integer.MAX_VALUE) || (!template.isStackable() && (count > 1)))
			{
				Util
					.handleIllegalPlayerAction(
						player,
							"Warning!! Character " + player.getName() + " of account " + player.getAccountName() + " tried to purchase invalid quantity of items at the same time.",
							Config.DEFAULT_PUNISH);
			...
			if ((subTotal + tax) > Integer.MAX_VALUE)
			{
				Util
					.handleIllegalPlayerAction(
						player,
							"Warning!! Character " + player.getName() + " of account " + player.getAccountName() + " tried to purchase over " + Integer.MAX_VALUE + " adena worth of goods.",
							Config.DEFAULT_PUNISH);
				return;
			}
