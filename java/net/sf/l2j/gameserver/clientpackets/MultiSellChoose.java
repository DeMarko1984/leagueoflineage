	...
	private void doExchange(L2PcInstance player, MultiSellEntry templateEntry, boolean applyTaxes, boolean maintainEnchantment, int enchantment)
	{
		...
		for (MultiSellIngredient e : _ingredientsList)
		{
			...
			if (e.getItemId() != 65336)
			{
				...
				if (inv.getInventoryItemCount(e.getItemId(), maintainEnchantment ? e.getEnchantmentLevel() : -1) < ((Config.ALT_BLACKSMITH_USE_RECIPES || !e.getMantainIngredient())
			...
			else
			{
				...
				if (clan.getReputationScore() < e.getItemCount())
				{
		...
		for (MultiSellIngredient e : entry.getIngredients())
		{
			...
			{
				int repCost = clan.getReputationScore() - e.getItemCount();
				clan.setReputationScore(repCost, true);
		...
		Deque<L2ItemInstance> addItems = new ArrayDeque<>();
		...
		// drop items if slots left
		while (!inv.validateCapacity(1) && !addItems.isEmpty())
		{
			inv.dropItem("Multisell", addItems.removeLast(), player, player.getTarget());
		}
