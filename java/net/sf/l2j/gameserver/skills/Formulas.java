...
public final class Formulas
{
	...
	public static int MAX_STAT_VALUE = Config.GetFormulasConfig().getMaxStatValue();
	public static double MAX_HP_MOD = Config.GetFormulasConfig().getMaxHPMod();
	public static double MAX_MP_MOD = Config.GetFormulasConfig().getMaxMPMod();
	public static double MAX_CP_MOD = Config.GetFormulasConfig().getMaxCPMod();
	
	private static double[] STRCompute = new double[]
	{
		1.036,
		34.845
	}; // {1.016, 28.515}; for C1
	private static double[] INTCompute = new double[]
	{
		1.020,
		31.375
	}; // {1.020, 31.375}; for C1
	private static double[] DEXCompute = new double[]
	{
		1.009,
		19.360
	}; // {1.009, 19.360}; for C1
	private static double[] WITCompute = new double[]
	{
		1.050,
		20.000
	}; // {1.050, 20.000}; for C1
	private static double[] CONCompute = new double[]
	{
		1.030,
		27.632
	}; // {1.015, 12.488}; for C1
	private static double[] MENCompute = new double[]
	{
		1.010,
		-0.060
	}; // {1.010, -0.060}; for C1
	
	protected static double[] WITbonus = new double[MAX_STAT_VALUE];
	protected static double[] MENbonus = new double[MAX_STAT_VALUE];
	protected static double[] INTbonus = new double[MAX_STAT_VALUE];
	protected static double[] STRbonus = new double[MAX_STAT_VALUE];
	protected static double[] DEXbonus = new double[MAX_STAT_VALUE];
	protected static double[] CONbonus = new double[MAX_STAT_VALUE];
	...
	static class FuncAddLevel3 extends Func
	{
		...
		{
			if (!env.isNoWeakPlayer())
			{
				return;
			}
			env.value += env.player.get().getLevel() / 3.0;
		}
	}
	...
	static class FuncMultLevelMod extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value *= env.player.get().getLevelMod();
		}
	}
	...
	static class FuncPAtkMod extends Func
	{
		...
		public void calc(Env env)
		{
			...
			env.value *= STRbonus[env.player.get().getSTR()] * env.player.get().getLevelMod();
		}
	}
	...
	static class FuncMAtkMod extends Func
	{
		...
		public void calc(Env env)
		{
			...
			double intb = INTbonus[env.player.get().getINT()];
			double lvlb = env.player.get().getLevelMod();
	...
	static class FuncMDefMod extends Func
	{
		...
		public void calc(Env env)
		{
			...
			if (env.player.get() instanceof L2PcInstance)
			{
				L2PcInstance p = (L2PcInstance) env.player.get();
			...
			env.value *= MENbonus[env.player.get().getMEN()] * env.player.get().getLevelMod();
		}
	}
	...
	static class FuncHennaSTR extends Func
	{
		...
		public void calc(Env env)
		{
			...
			L2PcInstance pc = (L2PcInstance) env.player.get();
			if (pc != null)
			{
	...
	static class FuncMaxHpAdd extends Func
	{
		...
		public void calc(Env env)
		{
			...
			L2PcTemplate t = (L2PcTemplate) env.player.get().getTemplate();
			int lvl = env.player.get().getLevel() - t.classBaseLevel;
	...
	static class FuncMaxHpMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			L2PcInstance p = (L2PcInstance) env.player.get();
			env.value *= CONbonus[p.getCON()] * MAX_HP_MOD;
		}
	}
	...
	static class FuncMaxCpMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			L2PcInstance p = (L2PcInstance) env.player.get();
			env.value *= CONbonus[p.getCON()] * MAX_CP_MOD;
		}
	}
	...
	static class FuncMaxMpMul extends Func
	{
		...
		public void calc(Env env)
		{
			...
			L2PcInstance p = (L2PcInstance) env.player.get();
			env.value *= MENbonus[p.getMEN()] * MAX_MP_MOD;
		}
	}
	...
	{
		// for LGE games
		if (target instanceof L2LgeTowerInstance)
		{
			return target.getMaxHp() * 0.10;
		}
	...
	public final double calcPhysDam(L2Character attacker, L2Character target, L2Skill skill, boolean shld, boolean crit, boolean dual, boolean ss)
	{
		...
		if (shld)
		{
			if (((100 - Config.ALT_PERFECT_SHLD_BLOCK) < Rnd.get(100)) || (target.getWaitableSuccessRate(WaitableSuccessTypes.PerfectShieldUses) >= 100))
			{
				damage = 1;
				target.sendPacket(new SystemMessage(SystemMessageId.YOUR_EXCELLENT_SHIELD_DEFENSE_WAS_A_SUCCESS));
				target.clearWaitableSuccessRate(WaitableSuccessTypes.PerfectShieldUses);
			}
			else
			{
				target.incWaitableSuccessRate(WaitableSuccessTypes.PerfectShieldUses, Config.ALT_PERFECT_SHLD_BLOCK);
			}
		}
	 ...
	 * @return
	 */
	public final boolean calcCrit(L2Character activeChar, double rate, boolean skillCrit)
	{
		if ((rate > Rnd.get(1000)) || (activeChar.getWaitableSuccessRate(WaitableSuccessTypes.CritHits) >= 1000))
		{
			activeChar.clearWaitableSuccessRate(WaitableSuccessTypes.CritHits);
			return true;
		}
		
		activeChar.incWaitableSuccessRate(WaitableSuccessTypes.CritHits, rate);
	...
	{
		double blowRate = activeChar.calcStat(Stats.BLOW_RATE, chance * (1.0 + ((activeChar.getDEX() - 20) / 100)), target, null);
		if ((blowRate > Rnd.get(100)) || (activeChar.getWaitableSuccessRate(WaitableSuccessTypes.BlowHits) >= 100))
		{
			activeChar.clearWaitableSuccessRate(WaitableSuccessTypes.BlowHits);
			return true;
		}
		
		activeChar.incWaitableSuccessRate(WaitableSuccessTypes.BlowHits, blowRate);
	 ...
	 * @return
	 */
	public final boolean calcLethal(L2Character activeChar, L2Character target, int baseLethal)
	{
		double rate = activeChar.calcStat(Stats.LETHAL_RATE, (baseLethal * ((double) activeChar.getLevel() / target.getLevel())), target, null);
		
		if ((rate > Rnd.get(100)) || (activeChar.getWaitableSuccessRate(WaitableSuccessTypes.Lethal) >= 100))
		{
			activeChar.clearWaitableSuccessRate(WaitableSuccessTypes.Lethal);
			return true;
		}
		
		activeChar.incWaitableSuccessRate(WaitableSuccessTypes.Lethal, rate);
	...
	{
		if ((mRate > Rnd.get(1000)) || (activeChar.getWaitableSuccessRate(WaitableSuccessTypes.MagicCritHits) >= 1000))
		{
			activeChar.clearWaitableSuccessRate(WaitableSuccessTypes.MagicCritHits);
			return true;
		}
		
		activeChar.incWaitableSuccessRate(WaitableSuccessTypes.MagicCritHits, mRate);
	...
	public boolean calcHitMiss(L2Character attacker, L2Character target)
	{
		...
		if (acc_attacker >= evas_target)
		{
			// 5% to miss
			return Rnd.get(100) >= 95;
		}
		return Rnd.get(100) >= ((evas_target / acc_attacker) * 99);
	}
	...
	public boolean calcShldUse(L2Character attacker, L2Character target)
	{
		...
		if (target.calcStat(Stats.SHIELD_AEGIS, 0, attacker, null) <= 0)
		{
		...
		if ((shldRate > Rnd.get(100)) || (target.getWaitableSuccessRate(WaitableSuccessTypes.ShieldUses) >= 100))
		{
			target.clearWaitableSuccessRate(WaitableSuccessTypes.ShieldUses);
			return true;
		}
		
		target.incWaitableSuccessRate(WaitableSuccessTypes.ShieldUses, shldRate);
	...
	public boolean calcMagicAffected(L2Character actor, L2Character target, L2Skill skill)
	{
		...
		if (target.isRaid() && ((type == SkillType.CONFUSION) || (type == SkillType.MUTE) || (type == SkillType.PARALYZE) || (type == SkillType.ROOT) || (type == SkillType.FEAR)
			|| (type == SkillType.SLEEP) || (type == SkillType.STUN) || (type == SkillType.DEBUFF) || (type == SkillType.AGGDEBUFF)))
		{
	...
	public boolean calcSkillSuccess(L2Character attacker, L2Character target, L2Skill skill, boolean ss, boolean sps, boolean bss)
	{
		...
		// protect timers
		if (!target.canAffectAbnormalEffect(type))
		{
			return false;
		}
		...
		if (Config.DEVELOPER)
		{
			System.out
				.println(
					skill.getName() + ": " + value + ", " + statmodifier + ", " + lvlmodifier + ", " + resmodifier + ", "
						+ ((int) (Math.pow((double) attacker.getMAtk(target, skill) / target.getMDef(attacker, skill), 0.2) * 100) - 100) + ", " + ssmodifier + " ==> " + rate);
		}
		return (Rnd.get(100) < rate);
	}
	...
	{
		// max 80 lvls
		// only if lvl depend over 9lv
		double diff = target.getLevel() - (attacker.getLevel() + 4);
		return (diff <= 0) || (Rnd.get(100) > diff);
	}
	...
	{
		STRCompute = Config.GetFormulasConfig().getSTRCompute();
		INTCompute = Config.GetFormulasConfig().getINTCompute();
		DEXCompute = Config.GetFormulasConfig().getDEXCompute();
		WITCompute = Config.GetFormulasConfig().getWITCompute();
		CONCompute = Config.GetFormulasConfig().getCONCompute();
		MENCompute = Config.GetFormulasConfig().getMENCompute();
		
		MAX_STAT_VALUE = Config.GetFormulasConfig().getMaxStatValue();
		WITbonus = new double[MAX_STAT_VALUE + 1];
		MENbonus = new double[MAX_STAT_VALUE + 1];
		INTbonus = new double[MAX_STAT_VALUE + 1];
		STRbonus = new double[MAX_STAT_VALUE + 1];
		DEXbonus = new double[MAX_STAT_VALUE + 1];
		CONbonus = new double[MAX_STAT_VALUE + 1];
		...
		MAX_HP_MOD = Config.GetFormulasConfig().getMaxHPMod();
		MAX_MP_MOD = Config.GetFormulasConfig().getMaxMPMod();
		MAX_CP_MOD = Config.GetFormulasConfig().getMaxCPMod();
	}
}
