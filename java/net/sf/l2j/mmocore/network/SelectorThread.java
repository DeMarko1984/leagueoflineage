 ...
 */
package net.sf.l2j.mmocore.network;
...
import net.sf.l2j.util.FixedSizeByteBufferPool;
...
public final class SelectorThread<T extends MMOClient<?>> extends Thread
{
	...
	private final FixedSizeByteBufferPool _bufferPool;
	// Pending Close
	private final NioNetStackList<MMOConnection<T>> _pendingClose;
	
	private boolean _shutdown;
	
	public SelectorThread(final SelectorConfig sc, final IMMOExecutor<T> executor, final IPacketHandler<T> packetHandler, final IClientFactory<T> clientFactory, final IAcceptFilter acceptFilter)
		throws IOException
	{
		...
		_bufferPool = new FixedSizeByteBufferPool((byte) HELPER_BUFFER_COUNT, HELPER_BUFFER_SIZE, BYTE_ORDER);
	...
	{
		ByteBuffer pool = _bufferPool.remove();
		if (pool == null)
		{
